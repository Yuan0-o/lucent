...
ENTRIES = [
    ...
    ("webSearchLocalDisabledHint", "Web search is unavailable while the local assistant is on — it answers offline. Your previous setting comes back when you turn it off.", "本地助手已开启，网络搜索不可用——它完全离线作答。关闭本地助手后会恢复你上一次的设置。", "ローカルアシスタントがオンの間、ウェブ検索は使用できません（完全オフラインで応答します）。オフにすると以前の設定に戻ります。", "로컬 어시스턴트가 켜져 있는 동안에는 웹 검색을 사용할 수 없습니다(오프라인으로 답변). 끄면 이전 설정으로 돌아갑니다."),
    ("memoryHighLocalDisabledHint", "The high memory tier is unavailable while the local assistant is on — a long prompt is more than an on-device model can handle well. Your previous setting comes back when you turn it off.", "本地助手已开启，高记忆档不可用——过长的上下文超出端侧模型的处理能力。关闭本地助手后会恢复你上一次的设置。", "ローカルアシスタントがオンの間、記憶「高」は使用できません。長すぎるプロンプトは端末上のモデルには重すぎます。オフにすると以前の設定に戻ります。", "로컬 어시스턴트가 켜져 있는 동안에는 높은 메모리 단계를 사용할 수 없습니다. 너무 긴 프롬프트는 온디바이스 모델이 감당하기 어렵습니다. 끄면 이전 설정으로 돌아갑니다."),
    ...
    ("attachmentsSectionTitle", "Attachments", "附件", "添付ファイル", "첨부 파일"),
    ("attachmentsSectionHint", "Images, PDFs, audio or any other file.", "图片、PDF、音频或任意其他文件。", "画像・PDF・音声など、どんなファイルでも。", "이미지, PDF, 오디오 등 모든 파일."),
