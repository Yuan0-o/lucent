 ...
 *
 * Desktop twin of the Android FontStore — same manifest shape, same ids, same on-disk layout
 * under the app data directory, so a backup made on one platform restores on the other. The
 * only adaptation is the import source (a plain [java.io.File]; see [import]).
 *
...
object FontStore {
    ...
    // Desktop adaptation: the source is a plain java.io.File from the system file dialog instead
    // of an Android content resolver stream; the verify/copy pipeline below is untouched.
    fun import(context: Context, source: File, customName: String? = null): FontSlot {
        ...
        val id = newId()
        val pickedName = source.name.ifBlank { "font" }
